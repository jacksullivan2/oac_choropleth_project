# -*- coding: utf-8 -*-
"""
Created on Thu Aug 12 12:22:18 2021
...
def choropleth(data_filepath, excel_sheet_name, fname, client_data_postcode_column_header, postcode_lad_filepath, shapefile_filepath, weighted_column, weighted_column_type):
    ...
    mv_dataframe = a.groupby(['LAD'], as_index=False)['SumAssured'].sum()
    ...
    # columns. 
    merged_gdf_df = pd.merge(map_df, mv_pc_df, on="LAD")
    ...
    # Set the subtitle 
    plt.suptitle(f"Data plotted for {len(a)} postcodes in {len(merged_gdf_df['LAD'])} Local Authority Districts", fontsize=7)
